...
class VerticalBarChart extends BarChart {
     ...
     * @param <type> $width
     * @param <type> $height
     * @param <type> $top Padding superior
     * @param <type> $right
     * @param <type> $bottom
     * @param <type> $left
     * @param <type> $logo 
    ...
    public function VerticalBarChart($width = 600, $height = 250, $top = 5, $right = 30, $bottom = 50, $left = 50, $logo = NULL) {
        parent::BarChart($width, $height, $logo);
        $this->emptyToFullRatio = 1 / 5;
        $this->plot->setGraphPadding(new Padding($top, $right, $bottom, $left));
    }
    ...
    protected function printAxis() {
        ...
        for ($value = $minValue; $value <= $maxValue; $value += $stepValue) {
            ...
            if (in_array($value, $this->highlightedValues)) {
                imagerectangle($img, $graphArea->x1 - 1, $y, $graphArea->x2, $y, $palette->red->getColor($img));
            }
        }
    ...
    /**
     * Print the bars.
     * @param <type> $labelBar Puede ser: NULL, un valor porcentual, o cualquier otro tipo de dato. Si es null, se
     *               omite una etiqueta en la parte superior de las barras, si es value, se coloca el valor que
     *               representa la barra y si es un valor porcentual, se coloca el porcentaje que representa la barra.
     *               Por ejemplo si la escala es de [0..5] el valor porcentual esperado es 20, así un valor de 3.5,
     *               será marcado con 70%. Si labelBar contiene una cadena cualquiera, por ejemplo 'valores'
     *               se mostrará en la parte superior de cada barra, el valor que dicha barra representa.
     */
    protected function printBar($labelBar = NULL) {
