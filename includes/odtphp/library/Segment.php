...
class Segment implements IteratorAggregate, Countable {
    ...
    public function setImage($key, $value, $width=0, $height=0) {
        ...
        if ($size === false) {
            throw new OdfException("Imagen inválida");
        }
        if (($width == 0) && ($height == 0)) {
            ...
            if ($extensionArchivo == 'gif') {
                ...
                $imageX = 'imagejpeg';
            } else if ($extensionArchivo == 'png') {  // OJO como la creación del PNG puso problema, la figura redimensionada se crea como JPG
			...
			imagecopyresized($tn, $image, 0, 0, 0, 0, $widthp, $heightp, $owidth, $oheight);
            $imageX($tn, $save, 100);
            $value = $save;
            $filename = strtok(strrchr($value, '/'), '/.');
            $file = substr(strrchr($value, '/'), 1);
